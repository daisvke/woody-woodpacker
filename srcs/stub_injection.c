...
Elf64_Ehdr *_elf_header, off_t _injection_offset)
{
	...
	if (_last_text_shdr) _last_text_shdr->sh_size += sizeof(_stub);
...
void	_ww_generate_new_file_with_parasite(off_t _injection_offset)
{
	...
	if (!_stub_with_pad) _ww_print_errors(_WW_ERR_ALLOCMEM);
	...
	if (!_file_with_stub) _ww_print_errors(_WW_ERR_ALLOCMEM);
